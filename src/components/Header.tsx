...
export default function Header() {
  ...
  return (
    <>
    {/* Top Announcement Bar for Payment on Delivery & Fast Courier */}
    <div className="bg-gradient-to-r from-emerald-950 via-[#0A0A0A] to-emerald-950 border-b border-emerald-500/20 text-white text-[11px] py-2 px-4 font-sans select-none">
      <div className="max-w-7xl mx-auto w-full flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 font-medium shrink-0">
          <span className="bg-emerald-500/20 text-emerald-400 font-mono text-[9px] font-extrabold px-2 py-0.5 rounded border border-emerald-500/30 uppercase tracking-wider flex items-center gap-1">
            <Truck className="w-3.5 h-3.5 text-emerald-400" />
            PAYMENT ON DELIVERY AVAILABLE
          </span>
          <span className="text-white/80 hidden md:inline">
            Inspect your package upon arrival before paying via Cash or M-Pesa nationwide!
          </span>
          <span className="text-white/80 md:hidden">
            Inspect first, pay upon delivery!
          </span>
        </div>
        <div className="flex items-center gap-3 shrink-0 font-mono text-[10px] text-white/60">
          <span className="hidden sm:inline">📞 Orders Hotline: <strong className="text-emerald-400">+254 792 620 789</strong></span>
          <button 
            type="button"
            onClick={() => setActiveView("checkout")} 
            className="text-[#C5A059] hover:text-white font-bold cursor-pointer transition-colors flex items-center gap-1"
          >
