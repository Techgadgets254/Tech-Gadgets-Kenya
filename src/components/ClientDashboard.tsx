...
export default function ClientDashboard() {
  ...
  const handleDownloadStatementPDF = async () => {
    ...
    doc.text(`Logon Primary Email: ${user?.email || "N/A"}`, 15, 75);
    doc.text(`Active Session ID: ${user?.uid ? user.uid.substring(0, 12) : "GUEST"}...`, 15, 80);
    ...
    doc.save(`Tech_Sokoni_Kenya_Tax_Statement_${user?.uid ? user.uid.substring(0, 6) : "GUEST"}.pdf`);
  };
      ...
      <div className="bg-[#0F0F0F] border border-white/10 rounded-3xl p-8 sm:p-12 text-center max-w-md mx-auto shadow-2xl animate-fadeIn my-8">
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
          <button
            ...
            className="w-full sm:w-auto bg-[#C5A059] hover:bg-[#C5A059]/90 text-black font-sans text-xs font-bold px-5 py-2.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2"
          >
            <span>Sign In with Google</span>
          </button>
          <button
            onClick={() => setActiveView("home")}
            className="w-full sm:w-auto bg-white/5 hover:bg-white/10 text-white font-sans text-xs font-semibold px-4 py-2.5 rounded-xl transition-all cursor-pointer border border-white/10"
          >
            Return to Homepage
          </button>
        </div>
                  ...
                  <div className="pt-4 border-t border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 shadow-xs/10">
                    ...
                    <div className="p-4 bg-[#C5A059]/10 border border-[#C5A059]/20 rounded-2xl max-w-sm shrink-0">
                      ...
                      <div className="space-y-1 text-[10px] font-mono text-[#C5A059]/80 leading-tight">
                        <p>Payment: <span className={activeOrder.paymentStatus === "Paid" ? "text-emerald-400 font-bold" : activeOrder.paymentStatus === "Failed" ? "text-red-500 font-bold" : "text-[#C5A059]"}>{String(activeOrder.paymentStatus || "PENDING").toUpperCase()}</span></p>
                        <p>Fulfillment: <span className={activeOrder.shippingStatus === "Delivered" ? "text-emerald-400 font-bold" : activeOrder.shippingStatus === "Shipped" ? "text-blue-400 font-medium" : "text-[#C5A059]"}>{String(activeOrder.shippingStatus || "PROCESSING").toUpperCase()}</span></p>
